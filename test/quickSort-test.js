...
let huge750000 = hugeArray.slice(0,750000);
...
describe('quickSort', function() {
  ...
  it('Should sort a small test array', function(){
    assert.deepEqual(quickSort(smallTestArray), [0,1,2,3,4,5,9,11,17,38]);
  });
  it('Should sort a big array', function(){
    assert.deepEqual(quickSort(largeSortArray),largeSortArray.sort((a,b) => a-b));
  });
  it('Should sort an array with 10,000 numbers in it', function (){
    assert.deepEqual(quickSort(huge10000),huge10000.sort((a,b) => a-b));
    // console.log(speedTest(quickSort,huge10000));
  });
  it('Should sort an array with 20,000 numbers in it', function (){
    assert.deepEqual(quickSort(huge20000),huge20000.sort((a,b) => a-b));
    // console.log(speedTest(quickSort,huge10000));
  });
  it('Should sort an array with 100,000 numbers in it', function (){
    assert.deepEqual(quickSort(huge100000),huge100000.sort((a,b) => a-b));
    // console.log(speedTest(quickSort,huge200000));
  });
  it('Should sort an array with 750,000 numbers in it', function (){
    assert.deepEqual(quickSort(huge750000),huge750000.sort((a,b) => a-b));
  });
  it('Should work on an array of negative numbers', function() {
    let testNegative = [-5,-2,-3,-12,-20,-15,-9,-1,-4];
    assert.deepEqual(quickSort(testNegative), [-20,-15,-12,-9,-5,-4,-3,-2,-1]);    
  });
  it('Should work on an array with positives and negatives', function (){
    let mixedPosNeg = [-5,3,-2,-3,7,-12,2,6,-20,-15,1,13,-9,-1,-4];
    assert.deepEqual(quickSort(mixedPosNeg),[ -20, -15, -12, -9, -5, -4, -3, -2, -1, 1, 2, 3, 6, 7, 13 ]);    
  });
  it('Should also sort letters', function(){
    assert.deepEqual(quickSort(["d", "b", "a", "c"]),["a", "b", "c", "d"]);
  });
});